...
const Preview:React.FC<props> = ({postDetails, comments}) => {
    ...
    const deleteComment = (id : number) => {
        ...
        Inertia.post('/deleteComment', data, {
            forceFormData: true,
          });
    }
    ...
    <Layout>
        <div className='flex flex-col w-full'>
            <div className='md:w-[60%] w-[90%] mx-auto'>
                <div className=" bg-white font-[Poppins] py-7 px-9 mb-6 shadow-md rounded-xl mx-auto">
                    <div className="flex flex-row items-center justify-between h-[6rem]">
                        <div className="flex flex-col">
                        <h1 className="font-bold sm:text-2xl text-base w-full">{postDetails.title}</h1>
                ...
                <div>
                    {
                        Object.keys(comments).map((element : any, index) => {
                            return <div key={index}>
                                <div className='px-4 py-4 mt-3 shadow rounded-sm'>
                                    <div className='flex flex-row justify-between items-center'>
                                        <p>{comments[index].body}</p>
                                        <button onClick={(e : React.MouseEvent<HTMLButtonElement, MouseEvent>) => {deleteComment(comments[index].id);}} className='text-red-600'>Delete</button>
                                    </div>
                                </div>
                            </div>
                        })
